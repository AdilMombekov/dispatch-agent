...
### [ ] UX.2 Главное меню (Чат/Коворк/Код/Настройки) + эфемерная навигация
**Промпт:** Перестроить `MAIN_MENU` под 4 раздела. **Коворк = синоним чата/воркспейса** (решено юзером — не отдельная сложная логика). Эфемерная навигация: при переходе по меню прошлое меню-сообщение удаляется (есть `_delete_message` + `_msg_log`), грузится следующее — меньше шума. Настройки → текущий аккаунт + кнопка переключения (см. UX.3).
- Чат/Коворк → обычный AI-чат (текст → Haiku-роутер, уже есть).
- Код → `/c` / выбор папки+модели (claude).
- Настройки → аккаунт, модели, коннекторы, скилы, плагины.
