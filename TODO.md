...
### [ ] UX.3 Переключение аккаунтов через Chrome+Google (blocked: P3.12)
**Промпт:** Юзер выбрал **полную автоматизацию** (не мгновенный swap). Флоу: Настройки → текущий аккаунт + Логаут → выбор аккаунта → Accept → открыть общий Chrome → вход через Google → залогиниться в Claude → дальше выбор моделей/коннекторов/скилов/плагинов.
