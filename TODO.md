...
### [x] 14. Шаг 6 — планировщик для kind=scheduled
> ✅ Сделано 2026-06-03:
> - **Отклонение от плана:** НЕ APScheduler (новая зависимость → bundling в exe). Свой лёгкий `agent/orchestrator/scheduler.py` (тред, опрос 30с, без зависимостей).
> - Форматы: `every <N>m|h` (интервал) и `daily HH:MM` (ежедневно). Покрывает «каждое утро в 8:00» и «каждые N часов».
> - Шаблоны = строки в `tasks` со status='scheduled' + cron-спека. При срабатывании — enqueue свежей копии (kind переклассифицируется), шаблон остаётся, next пересчитывается. Recovery при рестарте (next считается заново от now).
> - Команды: `/every 30m <…>`, `/daily 08:00 <…>`, `/crons`, `/uncron <id>`.
> - **Тесты:** next_fire всех форм, цикл срабатывания (enqueue копии + пересчёт next), uncron. Wiring проверен.
...
## 🟣 UX (Telegram) — запросы пользователя (2026-06-03)

### [x] UX.0 Быстрые команды Telegram + единый /help
> ✅ Сделано: `BOT_COMMANDS` — единый источник; `setMyCommands` на старте (меню `/`), `/help` строится из него же (не разъедутся). Покрывает /q /c /tasks /cancel /dispatch /clean.

### [x] UX.1 /clean — Haiku чистит шум
> ✅ Сделано 2026-06-03:
> - Роллинг-буфер `_msg_log` (deque 60/чат): пишет id+текст своих сообщений (`_send_message`) и входящих (`_handle_update`).
> - `/clean` → `_clean_classify()` отдаёт буфер Haiku (ROUTER_MODEL), тот возвращает JSON-массив id шума (меню/статусы/подтверждения/`/start`), сохраняя содержательные Q&A. Удаляет через `_delete_message` (в приватном чате бот может удалять и свои, и входящие). Fail-safe: мусор/ошибка парсинга → ничего не удаляет.
> - **Тест:** парсинг id из JSON-в-прозе + fail-safe на garbage.

### [x] UX.2 Главное меню (Чат/Коворк/Код/Настройки) + эфемерная навигация
> ✅ Сделано 2026-06-03:
> - `MAIN_MENU` перестроен под 4 раздела (2×2): 💬 Чат · 🤝 Коворк · 💻 Код · ⚙️ Настройки.
> - **Эфемерная навигация через edit-in-place** (`_edit_menu` → editMessageText): одно сообщение-меню морфится при переходах, чат не копит старые меню. Fallback на новое сообщение, если правка не прошла. (Выбрал edit вместо delete+send — строго меньше шума.)
> - Чат → инфо «пиши текстом» (Haiku-роутер). Коворк → рабочее пространство оркестратора (Dispatch ON/OFF кнопкой, очередь, бюджет, «Мои задачи»). Код → Claude Code/терминал/приложения/установка/скриншот/о системе/Chrome. Настройки → аккаунт (+переключение), скилы, траты, обновить, пересканировать.
> - Везде «⬅️ Назад» → `nav:main`. Новые callback'и: `nav:*`, `dispatch:toggle`, `tasks:show`, `settings:skills`, `settings:spend`.
> - Сохранены все старые действия (ничего не потеряно). **Тест:** билдеры меню, структура MAIN_MENU, метка аккаунта.
> - ⏳ Модели/коннекторы/плагины в Настройках — заглушки на будущее (часть UX.3/account-флоу).

### [~] UX.3 Переключение аккаунтов — вариант A (безопасный) СДЕЛАН
> ✅ Сделано 2026-06-03 (вариант A — родной OAuth Claude Code, без скрейпа Google):
> - Настройки → 🔑 Аккаунт: список аккаунтов, тап = сделать активным (✓), у каждого кнопка «🔓 Войти».
> - `_launch_claude_login(n)`: ставит аккаунт активным + открывает **видимую консоль с `claude`** и `CLAUDE_CONFIG_DIR`=config_dir аккаунта. Claude Code сам открывает корректный OAuth-браузер (Google/Anthropic) — **мы НЕ печатаем креды Google**, значит ноль риска блокировки/детекта. Юзер завершает вход в окне.
> - Команда `/login` (вход в активный) + callback `acctlogin:N`. Очистка ANTHROPIC_API_KEY из env (форс подписочного логина).
> - **Тест:** wiring + offline smoke (handlers без сети не падают).
>
> ⏳ **Осталось (по желанию):**
> - **Вариант B** (полный автоввод email/пароля в Google через computer-use) — НЕ делал: риск лока аккаунта, тестировать только с юзером за экраном на тестовом аккаунте. Вариант A решает ту же задачу безопаснее.
> - Выбор **моделей/коннекторов/скилов/плагинов** из Telegram после входа — отдельная под-фича (сейчас это настраивается внутри самого Claude Code). Скилы уже есть (`/skills`).
